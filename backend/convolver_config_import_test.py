...
class Test(TestCase):
    ...
    def test_channels_factors_and_inversions_as_list(self):
        self.assertEqual(
            channels_factors_and_inversions_as_list("0.0 1.1 -9.9"),
            [(0, 1.0, False), (1, 0.1, False), (9, 0.9, True)],
        )
        # Straigth inversion
        # Note, the Convolver documentation says to use
        # -0.99999 and not -0.0 for this.
        self.assertEqual(
            channels_factors_and_inversions_as_list("-0.0 -0.99999"),
            [(0, 1.0, True), (0, 0.99999, True)],
        )
    ...
    def test_delays_and_mixers_are_imported(self):
        ...
        self.assertEqual(conf["mixers"]["Mixer in"]["channels"], {"in": 2, "out": 1})
        self.assertEqual(conf["mixers"]["Mixer out"]["channels"], {"in": 1, "out": 3})
        self.assertEqual(
            conf["pipeline"],
            [
                {
    ...
    def test_path_is_ignored_for_impulse_response_files(self):
        ...
        conf = ConvolverConfig(convolver_config).to_object()
        self.assertEqual(
            conf["filters"]["IR1.wav-0"]["parameters"]["filename"], "IR1.wav"
        )
        self.assertEqual(
            conf["filters"]["IR2.wav-0"]["parameters"]["filename"], "IR2.wav"
        )
        self.assertEqual(
            conf["filters"]["IR3.wav-0"]["parameters"]["filename"], "IR3.wav"
        )
