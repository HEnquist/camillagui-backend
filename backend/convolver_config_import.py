...
def filename_of_path(path: str) -> str:
    return split(path.replace('\\', '/'))[-1]


def channels_factors_and_inversions_as_list(channels_and_factors: str) -> list[tuple[int, float, bool]]:
    channels_and_fractions = [channel_and_fraction.split('.')
                              for channel_and_fraction
                              in channels_and_factors.split(' ')]
    return [
        (abs(int(channel)),
         float('0.' + fraction),
         channel[0] == '-')
...
class ConvolverConfig:
    ...
    def as_json(self) -> dict:
        return {
            ...
            'pipeline':
                self._input_delay_pipeline_steps()
    ...
    def _convolution_filter_definitions(self) -> dict:
        return {
            f.name(): {
                'type': 'Conv',
                'parameters': {'type': 'Wav', 'filename': f.filename, 'channel': f.channel_in_file}
            }
    ...
    def _delay_pipeline_steps(self, delays: list[int]) -> list[dict]:
        return [
            ...
            for channel, delay in enumerate(delays) if delay != 0
        ]
        ...
        return {
            'Mixer in': {
                ...
                'mapping': [
                    {'dest': f.channel,
                     'sources': [
                         {'channel': channel, 'gain': factor, 'scale': 'linear', 'inverted': invert}
        ...
        return {
            'Mixer out': {
                ...
                'mapping': [
                    {'dest': output_channel,
                     'sources': [
                         {'channel': f.channel, 'gain': factor, 'scale': 'linear', 'inverted': invert}
                         for f in self._filters
    ...
    def _filter_pipeline_steps(self) -> list[dict]:
        return [
            {'type': 'Filter', 'channel': f.channel, 'names': [f.name()], 'bypassed': None, 'description': None}
            for f in self._filters
        ]
